...
#pragma omp parallel //private(ban)  
            {
                //vector<bool> ban;
...
#pragma omp parallel for reduction(&& : Correct)
            ...
            else fprintf(stdout, "WARNING:The Coloing result contains conflict!\n");
        }
        delete []Q;
        delete []Qtmp;
        delete []RndValues;
...
#pragma omp parallel
    ...
    long NVer    = m_vi_Vertices.size()-1;//G->numVertices;
    long NEdge   = m_vi_Edges.size()/2;//G->numEdges;  
    int *verPtr  = &m_vi_Vertices[0];
    int *verInd  = &m_vi_Edges[0];
    //long *verPtr = G->edgeListPtrs;   //Vertex Pointer: pointers to endV
    //edge *verInd = G->edgeList;       //Vertex Index: destination id of an edge (src -> dest)
    int MaxDegree = m_i_MaximumVertexDegree; //maxDegree
    vector<int> vtxColor(NVer, -1);  //color firstly assigned to be -1;
    ...
    for(int i=0; i<NVer; i++)
        randValues[i]= double(rand())/(RAND_MAX+1.0);
...
#pragma omp parallel for
    ...
    printf("@\t%d\t", nT);    
    if (myConflicts > 0)
        printf("Failed\t"); //printf("Check - WARNING: Number of conflicts detected after resolution: %d \n\n", myConflicts);
    else
        printf("Success\t");//Check - SUCCESS: No conflicts exist\n\n");
